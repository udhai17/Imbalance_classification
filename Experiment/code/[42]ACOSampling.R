...
ACO <- function(training,testing,ITA=50,ant_n=50,rho=0.8,target.column=target.column){
  
  sigma <- 5 #RBF kernal parameter
  S_major <- training[training[,target.column]==0,]
  S_minor <- training[training[,target.column]==1,]
  ...
  for(i in 1:ITA){
    ...
    for(j in 1:ant_n){
      ...
      classifier <- svm(x = tmp_trn[,-target.column],y = tmp_trn[,target.column],cost = 500)
      mat <- table(predict(classifier,testing[,-target.column]),testing[,target.column])
...
ACOsampling<- function(data,target.column,ITA=50,ant_n=50,rho=0.8){
  
  maj <- data[data[,target.column]==0,]
  min <- data[data[,target.column]==1,]
  REC  <- vector("numeric",nrow(maj))
  names(REC) <- rownames(maj)
  
  for(rep in 1:5){
    select <- STRPart(data,target.column,0.66)
    S <- select[[1]]
    V <- select[[2]]
    name <-rownames(S[S[,target.column]==0,])
    tmp <- ACO(training = S,testing = V,target.column=target.column)$sel
...
fin <- ACOsampling(data=data,target.column=3)



plot(fin[,1:2],col=fin[,3]+1)
plot3d(fin[,1:2],col=fin[,3]+1)
